...
This skill sits **before** `requirement-workflow`. It buys the missing knowledge with a throwaway
...
> to the learning — the four checks in step 6 replace `quality-gate`.

## When to use

| Use it | Do not use it |
|---|---|
| Nobody can state the acceptance criteria | The requirement is known — go to `requirement-workflow` |
...
them to the project, the way `quality-gate` calibrates tools to a language.

| Project nature | Riskiest thing, usually | Prototype medium | Participant |
|---|---|---|---|
| Consumer / UI-led | Value, then usability | Clickable prototype — use `ui-exploration` | 5 target users per round |
...
- CI does not run it. Coverage ignores it. `quality-gate` never sees it.
...
4. The Gherkin handed to `requirement-workflow` step 2.
5. The glossary from `TERM`, handed to `domain-modeling`.
...
These scenarios are the *first draft* of the spec, not the last. `development-workflow` step 1 entails
more of them from the domain model.

## Handing off to `domain-modeling`
...
```
product-discovery  ──►  requirement-workflow  ──►  development-workflow  ──►  quality-gate
  (no gate,               (Gherkin)                  (domain-modeling →          (binding)
   throwaway code)                                    implementation)
```

- **`ui-exploration`** is one medium inside step 3 — use it when the risk is usability or visual
  direction and the answer needs several worlds compared, not one prototype tested.
- **`requirement-workflow`** consumes the discovery record at step 2. Discovery never produces code that
  ships.
- **`domain-modeling`** consumes the glossary.
- **`pmi-github-project`** holds a discovery effort as its own work item — a **Spike**, timeboxed,
