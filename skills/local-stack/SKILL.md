...
- **One command**, wired the way `quality-gate` is wired — `make up`, `npm run dev`, `docker compose
...
- **When a defect is traced to emulator drift**, that is an improvement-loop entry, not a shrug: it
...
- **`development-workflow`** — step 2 validates against this.
- **`quality-gate`** — dimension 6 runs against this, locally and in CI.
- **`e2e-suite`** — the suite targets it.
- **`deployment-pipeline`** — the same definitions should generate the deployed topology wherever
