...
`quality-gate` green in CI on the branch, acceptance scenarios passing with validation evidence, no
...
`security-gate`, PR-bound dimensions: SAST, secrets, dependencies, IaC. Images and runtime posture
...
Severity follows `security-gate`'s triage policy: Critical stops the line, High is fixed before the
...
`improvement-loop` entry filed with `file-finding`, not a fourth waiver.
...
- [ ] Security pass clean.
- [ ] `quality-gate` green on the merge commit.
...
- **`security-gate`** — step 2; the dimensions, tools and triage policy.
- **`development-workflow`** — hands off at H2. Send incomplete handoffs back.
- **`e2e-suite`** — receives at H3.
- **`file-finding`** — how a finding you are not fixing becomes a tracked item.
- **`improvement-loop`** — where repeated waivers and escaped defects go.
