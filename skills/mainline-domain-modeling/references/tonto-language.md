<!--
Vendored from the Tonto project (NEMO/UFES), MIT licensed.
Upstream: https://github.com/nemo-ufes/Tonto/blob/main/examples/Guidances/.cursor/rules/tonto-guidance.mdc
Vendored at Tonto v0.4.12 (2026-08-03). Content is upstream verbatim except:
Cursor `alwaysApply` frontmatter removed. See references/ATTRIBUTION.md.
-->
...
<!-- BEGIN spry-forge insertion — see ATTRIBUTION.md. Sourced from the official
     `tonto-cli add-skill --target claude` SKILL.md, which covers this and the
     1884-line grammar reference does not. -->

#### Reserved keywords — cannot be used as attribute names

These words are **reserved** and must not be used as attribute or datatype field names. This is a
frequent and confusing source of parse errors, because the failure surfaces at the *next* line.

**Declaration / stereotype keywords:**
`kind`, `collective`, `quantity`, `quality`, `mode`, `relator`, `type`, `powertype`, `subkind`,
`phase`, `role`, `historicalRole`, `category`, `mixin`, `phaseMixin`, `roleMixin`,
`historicalRoleMixin`, `event`, `situation`, `process`, `class`, `datatype`, `enum`, `relation`,
`genset`, `intrinsicMode`, `extrinsicMode`

**Structural keywords:**
`label`, `description`, `specializes`, `instanceOf`, `of`, `import`, `package`, `disjoint`,
`complete`, `general`, `specifics`, `categorizer`, `where`, `inverseOf`

| Intended name | Problem | Fix |
|---|---|---|
| `description` | Reserved (class description block) | `_description`, `summary`, `details` |
| `value` | Conflicts with internal usage | `_value`, `amount`, `measure` |
| `role` | Reserved (stereotype keyword) | `_role`, `participationRole`, `function` |
| `type` | Reserved (stereotype keyword) | `_type`, `category`, `classification` |
| `label` | Reserved (label block keyword) | `_label`, `displayName`, `title` |
| `class` | Reserved (stereotype keyword) | `_class`, `category`, `classification` |
...
Note that `value` and `type` are exactly the names a domain model reaches for most often, so expect
to hit this.

<!-- END spry-forge insertion -->
