...
This is the harvesting rule that `development-workflow`, `product-discovery`, `e2e-suite`,
`review-station` and `observability` all defer to.
...
- **`pmi-github-project`** — the board, the fields, the labels and the issue forms this skill writes
  into. Stand it up first.
- **`development-workflow`** — "file what you find, now" at build time.
- **`product-discovery`** — `ASSUM` observations become risks through this skill.
- **`e2e-suite`** — a quarantined flaky test files a ticket the same day.
- **`review-station`** — a repeated waiver files a `Platform` item against the rule.
- **`observability`** — an alert or error group becomes a work item the same way.
- **`improvement-loop`** — amendments to a check are `Platform` findings with an owner and a date.
