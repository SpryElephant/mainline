...
> re-implemented through `development-workflow` — never the prototype code.

This skill answers *which direction*. It does not answer *what should be built* — that is
`product-discovery`, which treats a UI exploration as one prototype medium among several and turns
