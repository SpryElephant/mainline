...
The checklist is **`development-workflow` step 4**. Read it there and verify each line.

What each line means in practice:

- **`quality-gate` green in CI on the branch** — check the actual run, not the last local result
...
- Findings filed (`file-finding`), not carried in someone's head.
...
wrong, that is a `Platform` item and an `improvement-loop` entry, not a reason to hand off.
