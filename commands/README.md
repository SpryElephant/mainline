...
Filing a finding is not a handoff — that is the `file-finding` skill, usable from inside any session.
...
**Never move a card past a failing check.** If a check is wrong, that is an `improvement-loop` entry
...
- **`.github/project-fields.json`** — the cached field and option IDs, the same cache `file-finding`
